...
#undef RIGHT_BROADCAST
  ...
  bool operator==(const Vec<T, Dim> &p) const{
    if(Dim != p.Dim)
      return false;
    for(int i = 0 ; i < Dim ; i ++)
...
template <int Dim>
inline Vec<double, Dim> floor(const Vec<double, Dim> &rhs) {
  Vec<double, Dim> res;
...
template <int Dim>
inline Vec<double, Dim> abs(const Vec<double, Dim> &rhs) {
  Vec<double, Dim> res;
...
inline double norm(double x, int nt) { return std::abs(x); }
...
template <class T, int Dim>
inline T dist(const Vec<T, Dim> &V1, const Vec<T, Dim> &V2) {
  return std::sqrt(dot(V1-V2,V1-V2));
}
