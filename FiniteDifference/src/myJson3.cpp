 ...
 * u(x,y) = exp(y + sin(x)) \n
 * domain: square minus a circle whoes center is at the boundary of the square \n 
...
int main()
{
    ...
    {
        genJson myjson(1/n);
        cstring path = cstring("../json/f3_N")+std::to_string(int(n))+cstring(".json");
        myjson.set_circ(Vec(0, 0.5), 0.15);
        myjson.set_fval(f);
        myjson.set_BV0(u);
